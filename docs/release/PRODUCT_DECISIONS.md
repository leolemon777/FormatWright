...
|---|---|---|---|---|---|
| 1 | Product name | keep `Anole` / rename | **Keep Anole** after a paid trademark search in target markets (US/EU/CN); `formatwright.com/.dev/.org/.io` are all taken, so pick `formatwright.app` / `getformatwright.com` or similar and update public metadata | Engineering preflight (2026-08-10, `NAME_CLEARANCE.md`) found no exact-name collision on GitHub/crates.io/npm/web; domains are taken but discoverable alternatives exist. Renaming later costs more the longer we wait | 🔴 |
