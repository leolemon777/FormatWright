...
6. **Versioned activation and rollback** (implemented as batch B2 on top of this ADR). The desktop registry keeps the active entry per engine plus a verifiable history; store directories per version are never deleted on activation. At startup, an active entry that fails re-verification falls back automatically to the most recent verifiable history entry instead of being skipped silently. Manual rollback re-verifies the target entry first and consults a downgrade policy (the keyring may pin a minimum engine version); running jobs keep the exact in-process paths their immutable plans already resolved.
