...
A `Trusted` signature alone never promotes a pack to certified: certification additionally requires the human-signed transitive review (§6 and `sources.json.review_status`), and real release key material has not been generated yet — all currently shipped packs are honestly `Unsigned`. Verification was proven on 2026-08-15 with a generated key against a copy of the real PDF Starter pack, including a third-party (PyNaCl) signer reproducing the canonical bytes byte-for-byte before signing, and distinct CLI rejections for `Unsigned`, tampered (`InvalidSignature`), and revoked manifests. The multi-version registry from ADR-0011 item 6 is implemented in `crates/core/src/engine_registry.rs`: one atomic active pointer per engine, rollback history derived from the content-addressed store, startup recovery that automatically falls back to the newest still-verifiable version (and reports `Activated`/`FellBack`/`Failed` outcomes instead of skipping silently), and manual rollback that re-verifies the target and blocks unauthorized downgrades with `POLICY_BLOCKED`.
