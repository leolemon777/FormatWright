...
| FW-FR-030–034 | JOB_RECOVERY | `crates/core/src/job_store.rs`, `application/job_execution.rs`, `application/bulk_jobs.rs`, `maintenance.rs`, `application_state.rs`, runner no-clobber publish, CLI jobs/maintenance/batch-images, Desktop queue/bulk commands | 10k disk-backed transaction/paging, batch/idempotency/selection/bulk regressions, reservation/transition races, no-clobber publish, forced crash, shared executor failures, online backup/restore/v3→v4 snapshot, full-state bundle round-trip/tamper/journal recovery, disk CLI E2E | In progress; R-001–R-007 are Closed. Schema v4 durable batch/selection/bulk audit, SQLite maintenance/concurrency, no-clobber publish, and application-state bundle have direct evidence. Multi-process soak, Desktop maintenance and cross-platform recovery remain |
...
| FW-NFR-002 | RESOURCE_SCHEDULER | SQLite queue, deterministic resource scheduler, `JobExecutionService`, `MaintenanceService`, bounded desktop projection | `DURABLE_QUEUE.md`, `QUEUE_BRIDGE.md`, `BATCH_SANDBOX.md`, `MIXED_SCHEDULER.md`, `JOB_EXECUTION_SERVICE.md`, `MAINTENANCE_SERVICE.md`, `SQLITE_CONCURRENCY.md` | 10k persistence/real structured conversion, shared run_window tests, SQLite maintenance/concurrency, real-window projection, bounded batch pause/resume, and nine-job mixed RSS/WAL/process overlap verified on Windows; full mixed 10k and cross-platform gates pending |
...
| Desktop E10 | UX_FLOWS | Tauri commands plus React workflow shell; queue via `JobExecutionService`/`QueueWindowControl`, bulk via `BulkJobService` | `DESKTOP_MVP.md`, `BATCH_SELECTION_BULK.md`, preset/execution evidence, Rust/TypeScript tests, production build | In progress; exact preview approval, report-before-terminal persistence, recoverable pause, per-job and stable-filter bulk actions, live list/paging/enqueue are verified. Recovery banner, batch browser/virtualization, shell integration, screen-reader audit, and usability study remain |
