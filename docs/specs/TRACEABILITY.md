...
| FW-FR-030–034 | JOB_RECOVERY | `crates/core/src/job_store.rs`, `crates/core/src/application/job_execution.rs` (`JobExecutionService`), `crates/core/src/maintenance.rs` (`MaintenanceService`), `crates/core/src/runner.rs` no-clobber publish, CLI `jobs` / `maintenance` / `convert --queue-only` / `batch-images`, Desktop queue window | 10k disk-backed transaction/paging, reservation/transition races, no-clobber file/directory publish, forced-crash, recursive batch pause/resume, shared run_window failures, online backup/restore/migration preflight | In progress; R-001–R-007 are Closed. SQLite maintenance, immediate writer transactions, independent-connection reservation/transition winners, and no-clobber final publish have direct regressions. Multi-process soak, batch selection, application-state bundle, Desktop maintenance and cross-platform recovery remain |
...
| Desktop E10 | UX_FLOWS | Tauri commands plus React workflow shell; queue via `JobExecutionService` + `QueueWindowControl` | `docs/testing/DESKTOP_MVP.md`, `docs/testing/PRESET_SANDBOX.md`, `docs/testing/JOB_EXECUTION_SERVICE.md`, Rust/TypeScript tests, production build | In progress; exact preview approval, report-before-terminal persistence, recoverable active pause, per-job Resume/Retry and live list/paging/enqueue are verified. Recovery banner, selection/bulk retry, shell integration, full screen-reader audit, and usability study remain |
