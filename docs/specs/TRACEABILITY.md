...
| FW-FR-030–034 | JOB_RECOVERY | `crates/core/src/job_store.rs`, `crates/core/src/application/job_execution.rs` (`JobExecutionService`), CLI `jobs` / `convert --queue-only` / `batch-images`, Desktop queue window | 10k disk-backed transaction/paging, reservation races, forced-crash, recursive batch pause/resume/reinspection, mixed bounded scheduler, shared run_window and in-flight failure tests | In progress; R-001 is Closed with callback-storage and worker-panic failure injection proving cancel/drain/Interrupted recovery. CLI and Desktop share JobExecutionService and two pause controls; R-002–R-007, recoverable immediate pause, multi-connection races and cross-platform recovery remain |
