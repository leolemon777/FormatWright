...
| FW-FR-050–053 | ENGINE_SUPPLY_CHAIN | `crates/engine-sdk`, Doctor, `crates/core/src/engine_pack.rs`, versioned pack store, production `EngineLocator`, Desktop capability snapshot | Live hashes/build flags, missing-engine diagnostics, pack integrity/runtime tamper tests, polluted-PATH negative test, `docs/testing/ENGINE_RESOLUTION.md`, `docs/testing/WINDOWS_STARTER.md`, offline clean-VM Starter conversions | In progress; R-008/R-009 are Fixed: Release uses exact activated paths, Windows packs reject script wrappers, the installer embeds pinned PDF/Media packs, first startup installs them atomically, and UI/backend gate routes from the same snapshot. Doctor/Planner/Plan/Report now share ADR-0011 certification derivation (Trusted + complete review only). Gate U host-side negatives cover missing pack, tamper, compatibility/protocol mismatch, revoke, leftover `.partial`, failed upgrade, and PATH/override losing to a registered pack. Clean-VM conversion, official release key ceremony, legal review and engine SBOMs remain |
...
| Desktop E10 | UX_FLOWS | Tauri commands plus React workflow shell; queue via `JobExecutionService`/`QueueWindowControl`, bulk via `BulkJobService`, recovery via `JobRecoveryService`, validation-only via `RevalidationService`; Windows NSIS classic shell hooks and single-instance forwarding | `DESKTOP_MVP.md`, `DESKTOP_ACCESSIBILITY.md`, `DESKTOP_RELEASE_CONVERSION.md`, `WINDOWS_EXPLORER_INTEGRATION.md`, `JOB_BROWSER_RENDERING.md`, `TRUTHFUL_PROGRESS.md`, `JOB_RECOVERY_SERVICE.md`, `BATCH_SELECTION_BULK.md`, preset/execution evidence, Rust/TypeScript tests, production/NSIS build | In progress; exact preview approval, report-before-terminal persistence, truthful stages/wait reasons without fabricated ETA, recovery banner with per-engine fallback/failure notices, recoverable pause, per-job and stable-filter bulk actions, hard-bounded/native-isolated list rendering, live paging/enqueue, audited exact staging cleanup, bounded export, trusted output reveal, validation-only, installed Windows classic Explorer cold/hot path routing, real-WebView automated AX/keyboard/200%-equivalent/RTL/contrast/motion checks, and Release-UI PDF→PNG plus PDF→JPG CDP conversions from per-format isolated processes with Pass reports and deterministic page naming are implemented. Clean-VM/Windows 11 modern menu, live screen-reader/physical-DPI audit, and usability study remain |
...
| Security/Packaging E11 | THREAT_MODEL, ENGINE_SUPPLY_CHAIN, release checklist | Network-denied runner/path policy, zero-socket observation harness, isolated fuzz workspace, scheduled sanitizer workflow, SPDX generators (application plus per-engine file inventory), locked-dependency audit, offline NSIS packaging/checksums, verified engine staging with SBOM/sources sidecar validation, privacy/user/recovery docs | `docs/testing/ZERO_NETWORK.md`, `docs/testing/WINDOWS_STARTER.md`, `docs/security/FUZZING.md`, `docs/security/DEPENDENCY_AUDIT.md`, `docs/release/SBOM.md`, `docs/release/WINDOWS_PACKAGING.md`, `PRIVACY.md` | In progress; Windows bounded fuzz/application SBOM/deterministic engine file SBOM/zero-vulnerability dependency/observational zero-network gates pass, an unsigned installer with embedded Starter resources builds under the standard configuration (no test DevTools arguments), and the enhanced current-user install smoke re-verifies both packs, all four supply-chain sidecars, and clean uninstall. Trusted-manifest signature verification per ADR-0011 (canonical bytes, Ed25519 keyring with revocation/expiry, six-state fail-closed CLI) and the versioned engine registry with startup auto-fallback, explicit failure reporting, and authorized-rollback/downgrade blocking are implemented and cross-validated against a third-party signer on a real Starter pack; real release key ceremony, certification threading into Doctor/Planner/UI, transitive engine SBOM/legal review, clean-VM installed conversion, OS-enforced isolation, signed installers and cross-platform campaigns remain |
...
The Windows classic Explorer registration ownership, exact argument contract, single-instance forwarding, build evidence, and isolated installed-smoke checklist are in `docs/testing/WINDOWS_EXPLORER_INTEGRATION.md`.
