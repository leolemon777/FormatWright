...
| FW-FR-030–034 | JOB_RECOVERY | `crates/core/src/job_store.rs`, `application/job_execution.rs`, `application/bulk_jobs.rs`, `maintenance.rs`, `application_state.rs`, runner no-clobber publish, CLI jobs/maintenance/batch-images, Desktop queue/bulk commands | 10k disk-backed transaction/paging, batch/idempotency/selection/bulk regressions, reservation/transition/claim races, four-process exact-once execution, real forced crash/recovery, no-clobber publish, shared executor failures, online backup/restore/v3→v4 snapshot, full-state bundle round-trip/tamper/journal recovery, disk CLI E2E | In progress; R-001–R-007 are Closed. Schema v4 durable batch/selection/bulk audit, SQLite maintenance/concurrency, round-robin selection, atomic queue claim, no-clobber publish, and application-state bundle have direct Windows evidence. Long power-loss soak, Desktop maintenance and cross-platform recovery remain |
