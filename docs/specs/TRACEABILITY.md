...
| Desktop E10 | UX_FLOWS | Tauri commands plus React workflow shell; queue via `JobExecutionService`/`QueueWindowControl`, bulk via `BulkJobService`, validation-only via `RevalidationService` | `DESKTOP_MVP.md`, `BATCH_SELECTION_BULK.md`, preset/execution evidence, Rust/TypeScript tests, production build | In progress; exact preview approval, report-before-terminal persistence, recovery banner, recoverable pause, per-job and stable-filter bulk actions, live list/paging/enqueue, bounded export, trusted output reveal, and validation-only are verified. Long-list virtualization, manual partial cleanup, shell integration, screen-reader audit, and usability study remain |
