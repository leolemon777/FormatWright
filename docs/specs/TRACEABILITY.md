...
| FW-FR-030–034 | JOB_RECOVERY | `crates/core/src/job_store.rs`, `crates/core/src/application/job_execution.rs` (`JobExecutionService`), CLI `jobs` / `convert --queue-only` / `batch-images`, Desktop queue window | 10k disk-backed transaction/paging, reservation races, forced-crash, recursive batch pause/resume/reinspection, mixed bounded scheduler, shared run_window and in-flight failure tests | In progress; R-001–R-007 are Closed: Plan approval, report/terminal ordering, recoverable active pause, Windows output identity, structured cancellation/process-tree drain, and Desktop live list/paging/enqueue have direct regressions. Full multi-connection reservation races, batch selection, maintenance backup/restore and cross-platform recovery remain |
