...
| Desktop E10 | UX_FLOWS | Tauri commands plus React workflow shell; queue via `JobExecutionService`/`QueueWindowControl`, bulk via `BulkJobService`, recovery via `JobRecoveryService`, validation-only via `RevalidationService` | `DESKTOP_MVP.md`, `JOB_RECOVERY_SERVICE.md`, `BATCH_SELECTION_BULK.md`, preset/execution evidence, Rust/TypeScript tests, production build | In progress; exact preview approval, report-before-terminal persistence, recovery banner, recoverable pause, per-job and stable-filter bulk actions, live list/paging/enqueue, audited exact staging cleanup, bounded export, trusted output reveal, and validation-only are verified. Long-list virtualization, shell integration, screen-reader audit, and usability study remain |
