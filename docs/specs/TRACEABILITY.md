...
| FW-FR-030–034 | JOB_RECOVERY | `crates/core/src/job_store.rs`, `crates/core/src/application/job_execution.rs` (`JobExecutionService`), `crates/core/src/maintenance.rs` (`MaintenanceService`), CLI `jobs` / `maintenance` / `convert --queue-only` / `batch-images`, Desktop queue window | 10k disk-backed transaction/paging, reservation races, forced-crash, recursive batch pause/resume/reinspection, mixed bounded scheduler, shared run_window failures, online backup/restore/migration preflight | In progress; R-001–R-007 are Closed. SQLite maintenance directly verifies full integrity, portable non-overwriting backup, isolated v2→v3 restore preflight, newer/corrupt refusal, pre-migration snapshot, confirmed transactional restore and CLI disk E2E. Full multi-connection reservation races, batch selection, application-state bundle, Desktop maintenance and cross-platform recovery remain |
...
| FW-NFR-002 | RESOURCE_SCHEDULER | SQLite queue, deterministic resource scheduler, `JobExecutionService`, `MaintenanceService`, bounded desktop projection | `DURABLE_QUEUE.md`, `QUEUE_BRIDGE.md`, `BATCH_SANDBOX.md`, `MIXED_SCHEDULER.md`, `JOB_EXECUTION_SERVICE.md`, `MAINTENANCE_SERVICE.md` | 10k persistence/real structured conversion, shared run_window tests, SQLite maintenance/recovery, real-window projection, bounded batch pause/resume, and nine-job mixed RSS/WAL/process overlap verified on Windows; full mixed 10k and cross-platform gates pending |
