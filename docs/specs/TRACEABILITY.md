...
| FW-FR-030–034 | JOB_RECOVERY | `crates/core/src/job_store.rs`, `application/job_execution.rs`, `application/bulk_jobs.rs`, `maintenance.rs`, runner no-clobber publish, CLI jobs/maintenance/batch-images, Desktop queue/bulk commands | 10k disk-backed transaction/paging, batch/idempotency/selection/bulk regressions, reservation/transition races, no-clobber publish, forced crash, shared executor failures, online backup/restore/v3→v4 snapshot, disk CLI bulk E2E | In progress; R-001–R-007 are Closed. Schema v4 durable batch/selection/bulk audit, SQLite maintenance/concurrency, and no-clobber publish have direct evidence. Multi-process soak, application-state bundle, Desktop maintenance and cross-platform recovery remain |
