...
| R-009 | P1 | Open | Release engine resolution falls back to ambient `PATH` and accepts `.cmd`/`.bat`, allowing broken or unrelated development wrappers to become the selected engine | `doctor.rs` lookup order and Windows executable-extension handling; Doctor selected Codex `override\\pdfinfo.cmd` and version check failed | Release build resolves only an activated, hash-verified pack by exact path; `.cmd`/`.bat` and ambient PATH are rejected in production; developer override remains explicitly opt-in and is covered by negative tests |
