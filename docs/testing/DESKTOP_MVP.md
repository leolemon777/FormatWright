...
- The Windows installer owns classic Explorer Open-in entries plus 17 Convert verbs generated from one table. Open-in (`--shell-open`) only pre-fills Convert and creates 0 Jobs. Convert to X (`--shell-convert --to`) is CLI-equivalent approval. The backend rejects missing, relative, UNC, device-namespace, and bare arguments. The single-instance plugin is registered first, queues follow-up paths, and focuses the existing window.
...
- `formatwright-desktop.exe` starts a native window titled `Anole`, remains responsive, and is then terminated by the harness.
